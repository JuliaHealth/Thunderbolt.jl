...
function _build_l1prec(builder::L1GSPrecBuilder, _A::MatrixType,partsize::Ti) where {Ti<:Integer,MatrixType}
    ...
    D_Dl1 = _precompute_blocks(A, partitioning)
    L1GSPreconditioner(partitioning,D_Dl1)
end
...
function _precompute_blocks(_A::AbstractSparseMatrix,partitioning::BlockPartitioning)
    ...
    kernel(D_Dl1,A,symA,partsize,nparts,nchunks,chunksize; ndrange=ndrange)
    synchronize(backend)
    return D_Dl1
end


@kernel function _precompute_blocks_kernel!(D_Dl1,A,symA,partsize::Ti,nparts::Ti,nchunks::Ti,chuncksize::Ti) where {Ti<:Integer}
