function residual_norm(cache::AbstractNonlinearSolverCache, f::AbstractSemidiscreteFunction)
    norm(cache.residual)
end
function residual_norm(cache::AbstractNonlinearSolverCache, f::AbstractQuasiStaticFunction)
...
function eliminate_constraints_from_increment!(
    ...
    for (i, fi) in enumerate(blocks(f))
