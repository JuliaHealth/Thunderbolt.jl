...
function semidiscretize(
    ...
    for subdomain in discretization.subdomains
        ...
        for (type, set) in descriptor.data 
            ...
            for cell in set
                ...
                for node_id in mesh.grid.cells[cell].nodes
                    coords = mesh.grid.nodes[node_id].x
                    ff = findfirst(node_id, nodes_non_zeros_indices)
                    if isnothing(ff)
    ...
    vertices = SparseVector(length(mesh.grid.vertices), nodes_non_zeros_indices, nodes_non_zeros_values)
