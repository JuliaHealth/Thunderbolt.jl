...
QuadratureIterator(fqr::FacetQuadratureRule, local_facet_idx::Int) =
...
) where {dim, T} = QuadraturePoint{dim, T}
...
Ferrite.shape_value(cv::CellValues, qp::QuadraturePoint, base_fun_idx::Int) =
    Ferrite.shape_value(cv, qp.i, base_fun_idx)
Ferrite.shape_gradient(cv::CellValues, qp::QuadraturePoint, base_fun_idx::Int) =
    Ferrite.shape_gradient(cv, qp.i, base_fun_idx)
Ferrite.function_value(cv::CellValues, qp::QuadraturePoint, ue) =
    Ferrite.function_value(cv, qp.i, ue)
Ferrite.function_gradient(cv::CellValues, qp::QuadraturePoint, ue) =
    Ferrite.function_gradient(cv, qp.i, ue)

Ferrite.getdetJdV(fv::FacetValues, qp::QuadraturePoint) = Ferrite.getdetJdV(fv, qp.i)
Ferrite.shape_value(fv::FacetValues, qp::QuadraturePoint, base_fun_idx::Int) =
    Ferrite.shape_value(fv, qp.i, base_fun_idx)
Ferrite.shape_gradient(fv::FacetValues, qp::QuadraturePoint, base_fun_idx::Int) =
    Ferrite.shape_gradient(fv, qp.i, base_fun_idx)
Ferrite.function_value(fv::FacetValues, qp::QuadraturePoint, ue) =
    Ferrite.function_value(fv, qp.i, ue)
Ferrite.function_gradient(fv::FacetValues, qp::QuadraturePoint, ue) =
...
Ferrite.function_value(cv::FerriteUtils.StaticInterpolationValues, qp::QuadraturePoint, ue) =
    Ferrite.function_value(cv, qp.i, ue)
