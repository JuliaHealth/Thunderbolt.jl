...
function generate_ideal_lv_mesh_hex(
    ...
    point(θ, φ, rp) = _ellipsoid_point(θ, φ, rp;
        inner_radius, outer_radius, apex_inner, apex_outer, septum_flatness, axis_ratio, eccentricity)

    # Wall, identical to the fan variant: rings from the one above the apex up to
    # the base, circumferential index fastest.
    nodes = Node{3,T}[]
    for θ ∈ longitudinal_angle[2:end], radius_percent ∈ radii_in_percent, φ ∈ circumferential_angle[1:(end-1)]
        push!(nodes, Node(point(θ, φ, radius_percent)))
    end
    node_array = reshape(collect(1:(n_nodes_c*n_nodes_r*n_nodes_l)), (n_nodes_c, n_nodes_r, n_nodes_l))
    ...
    for radius_percent ∈ radii_in_percent, b in 1:(m+1), a in 1:(m+1)
        X = lattice[a,b]
    ...
    ne_wall = num_elements_circumferential*num_elements_radial*num_elements_logintudinal
    cell_array = reshape(collect(1:ne_wall), (num_elements_circumferential, num_elements_radial, num_elements_logintudinal))
    facetsets = Dict{String,OrderedSet{FacetIndex}}(
    ...
    for j in 1:num_elements_radial, i in 1:num_elements_circumferential
        ...
        push!(cells, Hexahedron((node_array[i,j,1],   node_array[i_next,j,1],   core_array[an,bn,j],   core_array[a,b,j],
    ...
    for j in 1:num_elements_radial, b in 1:m, a in 1:m
        push!(cells, Hexahedron((core_array[a,b,j],   core_array[a+1,b,j],   core_array[a+1,b+1,j],   core_array[a,b+1,j],
                                 core_array[a,b,j+1], core_array[a+1,b,j+1], core_array[a+1,b+1,j+1], core_array[a,b+1,j+1])))
        j == 1                   && push!(facetsets["Endocardium"], FacetIndex(length(cells), 1))
        j == num_elements_radial && push!(facetsets["Epicardium"],  FacetIndex(length(cells), 6))
    end
    ...
    nodesets = Dict{String,OrderedSet{Int}}(
        ...
        "MyocardialAnchor4" => OrderedSet{Int}([node_array[ceil(Int,1+3*n_nodes_c/4),1,end]]),
