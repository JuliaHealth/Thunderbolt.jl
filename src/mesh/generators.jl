...
function generate_ring_mesh(
    ...
    for k in 1:n_nodes_l, j in 1:n_nodes_r, i in 1:n_nodes_c
        ...
        radius = radial_coords[j] -
                 apicobasal_tilt * longitudinal_angle[k] / maximum(abs.(longitudinal_angle))
        push!(
            nodes,
            Node((
    ...
    cells = Hexahedron[]
    for k in 1:num_elements_logintudinal,
        j in 1:num_elements_radial,
        i in 1:num_elements_circumferential
    ...
    nodesets["MyocardialAnchor3"] = OrderedSet{Int}([node_array[
        ceil(Int, 1 + n_nodes_c / 4), 1, 1]])
    nodesets["MyocardialAnchor4"] = OrderedSet{Int}([node_array[
        ceil(Int, 1 + 3 * n_nodes_c / 4), 1, 1]])
...
function generate_open_ring_mesh(
    ...
    # Generate nodes
    circumferential_angle = range(
        opening_angle / 2, stop = 2 * π - opening_angle / 2, length = n_nodes_c)
    radial_coords = range(inner_radius, stop = outer_radius, length = n_nodes_r)
    longitudinal_angle = range(longitudinal_upper, stop = longitudinal_lower, length = n_nodes_l)
    nodes = Node{3, T}[]
    for k in 1:n_nodes_l, j in 1:n_nodes_r, i in 1:n_nodes_c
...
function generate_quadratic_ring_mesh(
    ...
    for k_ in 1:num_elements_logintudinal,
        j_ in 1:num_elements_radial,
        i_ in 1:num_elements_circumferential
...
function generate_ideal_lv_mesh(
    ...
    for θ in longitudinal_angle[2:(end - 1)],
        radius_percent in radii_in_percent,
        φ in circumferential_angle[1:(end - 1)]
        ...
        # cylindrical -> carthesian
        z = θ < π / 2 ?
            (apex_inner * (1.0 - radius_percent) + apex_outer * (radius_percent)) * cos(θ) :
            apex_outer * cos(θ)
        push!(nodes, Node((radius * cos(φ) * sin(θ), radius * sin(φ) * sin(θ), z)))
    end

    # Add flat base
    for θ in longitudinal_angle[end],
        radius_percent in radii_in_percent,
        φ in circumferential_angle[1:(end - 1)]
        ...
        # cylindrical -> carthesian
        push!(
            nodes, Node((radius * cos(φ) * sin(θ), radius * sin(φ) * sin(θ), apex_outer * cos(θ))))
    end
    ...
    nodesets["MyocardialAnchor3"] = OrderedSet{Int}([node_array[
        ceil(Int, 1 + n_nodes_c / 4), 1, end]])
    nodesets["MyocardialAnchor4"] = OrderedSet{Int}([node_array[
        ceil(Int, 1 + 3 * n_nodes_c / 4), 1, end]])
    ...
    for radius_percent in radii_in_percent
    ...
    # Add apex cells
    for j in 1:num_elements_radial, i in 1:num_elements_circumferential
        i_next = (i == num_elements_circumferential) ? 1 : i + 1
...
function generate_simple_disc_grid(::Type{Quadrilateral}, n; radius = 1.0)
    ...
    elements = Quadrilateral[
                             Quadrilateral((2i - 1 == 0 ? nnodes - 1 : 2i - 1, 2i,
                                 2i + 1 == nnodes ? 1 : 2i + 1, nnodes)) for
                             i in 1:n
                             ]

    facetsets = Dict(
        "boundary" => OrderedSet([FacetIndex(i, 1) for i in 1:n]) ∪
                      OrderedSet([FacetIndex(i, 2) for i in 1:n]),
    )
