...
function SciMLBase.build_solution(prob::AbstractSemidiscreteProblem,
        ...
        retcode = SciMLBase.ReturnCode.Default, destats = missing, stats = nothing,
        kwargs...)
    T = eltype(eltype(u))
    N = 2 # Why?
    ...
    return SciMLBase.ODESolution{T, N}(u,
        nothing,
        nothing,
        t, k,
        nothing,
...
QuasiStaticProblem(f::AbstractQuasiStaticFunction, tspan::Tuple{<:Real, <:Real}) = QuasiStaticProblem(f, zeros(solution_size(f)), tspan, SciMLBase.NullParameters())
QuasiStaticProblem(f::AbstractQuasiStaticFunction, u0::AbstractVector, tspan::Tuple{<:Real, <:Real}) = QuasiStaticProblem(f, u0, tspan, SciMLBase.NullParameters())
...
PointwiseODEProblem(f::AbstractPointwiseFunction, tspan::Tuple{<:Real, <:Real}) = PointwiseODEProblem(f, zeros(solution_size(f)), tspan, SciMLBase.NullParameters())
PointwiseODEProblem(f::AbstractPointwiseFunction, u0::AbstractVector, tspan::Tuple{<:Real, <:Real}) = PointwiseODEProblem(f, u0, tspan, SciMLBase.NullParameters())
...
ODEProblem(f::AbstractSemidiscreteFunction, tspan::Tuple{<:Real, <:Real})  = ODEProblem(f, zeros(ndofs(f.dh)), tspan, SciMLBase.NullParameters())
ODEProblem(f::AbstractSemidiscreteFunction, u0::AbstractVector, tspan::Tuple{<:Real, <:Real})  = ODEProblem(f, u0, tspan, SciMLBase.NullParameters())
