...
@testset "IO" begin
    ...
    @testset "CoBiVeC" begin
        ...
        for (cell1, cell2) in zip(CellIterator(dh), CellIterator(cobivec.dh))
            ...
            @test cobivec.u_apicobasal[celldofs(cell2)] ==
                  coordinate_data[all_dofs[dof_range(dh, :apicobasal)]]
            @test cobivec.u_transmural[celldofs(cell2)] ==
                  coordinate_data[all_dofs[dof_range(dh, :transmural)]]
            @test cobivec.u_rotational[celldofs(cell2)] ==
                  coordinate_data[all_dofs[dof_range(dh, :rotational)]]
            @test cobivec.u_transventricular[celldofs(cell2)] ==
                  coordinate_data[all_dofs[dof_range(dh, :transventricular)]]
        end
    end
end
